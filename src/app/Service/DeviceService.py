...
from adbutils import adb, AdbError, AdbTimeout, adb_path
...
def getAdbDeviceList():
    try:
        ...
        raise InvalidUsage(message="设备连接失败,重启ADB,请稍后重试\nlog:{}".format(e))
    if len(devices) == 0:
        raise InvalidUsage(message="没有连接的ADB设备")
...
def connectAdbDeviceRemote(address: str):
    try:
        ...
        raise InvalidUsage(message="连接超时哩\nlog:{}".format(e), status_code=500)
    if "fail" in msg or "cannot" in msg:
...
def disconnectAdbDeviceRemote(address):
    try:
        ...
        raise InvalidUsage(message="不存在该设备捏\nlog:{}".format(e))
    return success_response(message=msg)
...
def connectDevice(data):
    ...
    try:
        ...
        tmp_device = GameScript(screenshot_method=screenshot_method_ex, touch_method=touch_method_ex, debug=debug)
        tmp_device.screenshot()
        tmp_device.click(1, 1)
    except Exception as e:
        raise InvalidUsage(message="{}".format(e))
