...
class RegionalGhostKing:
    defaultConfig = {
        "挑战按钮": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/挑战.png", describe="地域鬼王挑战按钮"
        ),
        "今日挑战": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/今日挑战.png", describe="地域鬼王今日挑战按钮",
            region=[9, 310, 155, 479]
        ),
        "未选择": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/未选择.png", describe="未选择鬼王",
            region=[134, 349, 491, 450]
        ),
        "筛选": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/筛选.png", describe="未选择鬼王",
            region=[1084, 6, 1191, 126]
        ),
        "挑战": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/挑战.png", describe="挑战鬼王"
        ),
        "挑战鬼王按钮范围数组": [[1070, 213, 1165, 316], [1069, 368, 1167, 462], [1069, 524, 1169, 620]],
        "挑战2": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/挑战2.png", describe="挑战鬼王", region=[1045, 455, 1263, 644]
        ),
        "关闭": ImageTemplate(
            template_path=IMAGES_DIR + "/地域鬼王/粉色叉.png", region=[1180, 6, 1271, 102]
        ),
        "今日挑战动画延迟": 1,
        "筛选动画延迟": 1,
        "挑战按钮延迟": 1,
        ...
        if self.device.find_and_click(self.config["今日挑战"]):
            ...
            if self.device.find(self.config["今日挑战"]):
                if self.device.find(self.config["未选择"]):
                    ...
                    self.done = True
                    return
        self.device.find_and_click(self.config['挑战2'])
