...
class PersonalBreakthrough:
    defaultConfig = {
        "突破图标": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/结界突破.png", describe="突破图标",
            region=[200, 600, 400, 720]
        ),
        "刷新": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/刷新.png", describe="刷新按钮",
            region=[900, 550, 1200, 650]
        ),
        "刷新确认": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/刷新确认.png", describe="刷新确认按钮",
            region=[600, 300, 900, 500]
        ),
        "进攻": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/进攻.png", describe="进攻按钮",
            region=[100, 300, 1200, 700]
        ),
        "突破胜利标识": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/突破胜利标识.png", describe="突破胜利标识", threshold=0.8,
            region=[140, 140, 1200, 550]
        ),
        "防守记录": ImageColorTemplate(
            template_path=IMAGES_DIR + "/个人突破/防守记录.png", describe="防守记录",
            region=[100, 600, 200, 720]
        ),
        "关闭突破": ImageTemplate(
            template_path=IMAGES_DIR + "/个人突破/关闭突破.png", describe="关闭突破",
    ...
    def __init__(self, device: GameScript, updateConfig=None) -> None:
        ...
        self.settleTaskFail = SettleTask(self.device, SETTLE_FAIL, fightAgain=True)
    ...
    def __personalBreakthroughPageStage(self):
        if self.complete:
            self.device.find_and_click(self.config["关闭突破"])
        else:
            if self.device.find(self.config["防守记录"]):
                ...
                for index, region in enumerate(self.config["个人突破范围矩阵"]):
                    ...
                    if not self.device.find(self.config["突破胜利标识"]):
                        self.device.range_random_click(region)
                        self.winTimes = index
                        break
                if self.winTimes == 0:
                    self.failTimes = 0
            if self.device.find_and_click(self.config["进攻"]):
