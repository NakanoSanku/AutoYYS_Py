...
from src.tasks.settle import SettleTask
...
SIX_STREET_CONFIG = {
    # 开启六道界面
    "攻略": ImageTemplate(
        templatePath=IMAGES_DIR + "/六道/攻略.png", region=[5, 3, 567, 184],threshold=0.85
    ),
    "开启": ImageTemplate(
        templatePath=IMAGES_DIR + "/六道/开启.png", region=[1086, 540, 1257, 718],threshold=0.85
    ),
    "确定": ImageTemplate(
        templatePath=IMAGES_DIR + "/六道/确定.png", region=[1086, 540, 1257, 718],threshold=0.85
    ),
    "开启_2": ImageTemplate(
        templatePath=IMAGES_DIR + "/六道/开启_2.png", region=[1105, 546, 1271, 718],threshold=0.85
    ),
    # BUFF选择
    ...
    # 神秘岛屿
    "神秘之屿": ImageTemplate(
        templatePath=IMAGES_DIR + "/六道/神秘之岛.png", region=[66, 1, 281, 76],threshold=0.85
    ),
    # 背包仿造情况
    "背包仿造": ImageTemplate(
...
class SixStreet:
    def __init__(self, device: GameScript, times: int, config=None) -> None:
        self.config = SIX_STREET_CONFIG
        if config is None:
            config = {}
